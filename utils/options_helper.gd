...
func set_options():
  ...
  set_qte_mode(AppState.data.get(Constants.options, {}).get("qte_mode", BattleSceneHelper.QTEMode.TOUCH if OSHelper.is_mobile() else BattleSceneHelper.QTEMode.TOUCH_AND_BUTTON));
